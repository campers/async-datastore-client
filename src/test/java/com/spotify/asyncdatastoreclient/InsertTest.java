...
public class InsertTest extends DatastoreTest {
  ...
  public void testInsertEntity() throws Exception {
    ...
    final Entity address = Entity.builder("address", 222222L)
        .property("first_line", "22 Arcadia Ave").property("zipcode", "90210").build();

    final Entity entity = Entity.builder("employee").property("fullname", "Fred Blinge")
        .property("nickname", "Freddie", false).property("height", 2.43)
        .property("holiday_allowance", 22.5, false).property("payroll_number", 123456789)
        .property("age", 40, false).property("senior_role", false).property("active", true, false)
        .property("start_date", now).property("update_date", now, false)
        .property("picture", picture).property("address", address)
        .property("manager", Key.builder("employee", 234567L).build())
        .property("workdays", ImmutableList.of("Monday", "Tuesday", "Friday"))
        .property("overtime_hours", ImmutableList.of(2, 3, 4)).build();
  ...
  public void testInsertAlreadyExists() throws Exception {
    ...
    final Insert insertSecond = QueryBuilder.insert("employee", 1234567L)
        .value("fullname", "Jack Spratt").value("age", 21, false);
  ...
  public void testInsertBlob() throws Exception {
    ...
    final Insert insert = QueryBuilder.insert("employee").value("picture1", small)
  ...
  public void testInsertFullThenGet() throws Exception {
    ...
    final Insert insert = QueryBuilder.insert(key).value("fullname", "Fred Blinge")
        .value("age", 40, false);
  ...
  public void testInsertBatch() throws Exception {
    ...
    final Insert insert3 = QueryBuilder.insert(Key.builder("employee", parent).build())
        .value("fullname", "Harry Ramsdens").value("age", 50, false);

    final Batch batch = QueryBuilder.batch().add(insert1).add(insert2).add(insert3);

    final MutationResult result = datastore.execute(batch);
    assertFalse(result.getInsertKeys().isEmpty());

    final Query getAll = QueryBuilder.query().kindOf("employee")
        .filterBy(QueryBuilder.ancestor(parent)).orderBy(QueryBuilder.asc("fullname"));
